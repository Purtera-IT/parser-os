#!/usr/bin/env bash
# One A100 session: train both ceiling-bound heads, stream tqdm + per-epoch
# held-out metrics, then print a single combined verdict summary at the end.
# (The schematic symbol detector was retired 2026-08-26 — pure construction
# schematics are no longer accepted into Purpulse.)
set -uo pipefail

echo "Installing deps..."
pip install -U "transformers>=4.44" datasets accelerate scikit-learn tqdm 2>&1 | tail -1

echo "===================== 1/2  ATOM_TYPE HEAD (#70) ==========================="
python runpod_detector/train_type_head_gpu.py 2>&1 | tee _gpu_typehead.log

echo "===================== 2/2  SPAN TAGGERS (#71) ============================="
