"""SOWSmith — deterministic Statement of Work generator.

The Purtera-IT/SowSmith GitHub repo is currently an empty redirect to
parser-os. This module IS the SOWSmith implementation — it consumes
the OrbitBrief envelope (built by ``build_orbitbrief_envelope``) and
emits a contract-grade Statement of Work in markdown.

Related assets in the broader Purtera stack (loadable as inputs in
future expansions, but not required for this module to function):

  * ``purpulse.app/SOWSmith_policy_pack_v2.yaml`` (~12k lines) — the
    canonical clause modules + service-line requiredness logic. The
    parser-os primitives this module reads (pm_dashboard,
    sow_readiness_scorecard, srl_missing_checklist) are the exact
    inputs the policy pack is designed to reason over.
  * ``purpulse.app/SOW_field_catalog_v2.yaml`` (~10.6k lines) — the
    full 707-field SOW Requirements Library. The expanded SRL
    checklist in ``orbitbrief_core.build_srl_missing_checklist``
    currently covers a 41-field subset; the full catalog plugs in by
    replacing ``_SRL_REQUIRED_FIELDS`` with a YAML loader.
  * ``Orbitbrief-Core/src/orbitbrief_core/pm_handoff/render_html.py``
    — the LLM-side HTML PM handoff renderer (markdown -> HTML, with
    cards / tables / blockquotes styled for client delivery).
...
Design principles:
  ...
  * MISSING fields surface explicitly as ``[NEEDS DATA: field_id]``
    so a PM scanning the SOW sees the gaps instead of silently
    shipping incomplete contracts.
  * Markdown formatting is intentionally rich — headings, tables,
    audit-trail callouts — so the doc renders well in any markdown
    consumer (GitHub, Notion, VSCode, Slack, web).
  * No LLM in the hot path. An LLM polish layer (Orbitbrief-Core's
    pm_handoff or downstream) can soften phrasing, but the
    structural backbone + every receipt comes from this code.
"""
from __future__ import annotations

from typing import Any

SOW_VERSION = "sowsmith_v1"
...
def _section_header(envelope: dict[str, Any]) -> str:
    ...
    return (
        f"# Statement of Work — {_humanize(project_id)}\n\n"
        f"**Project ID:** `{project_id}`  \n"
        f"**Compile ID:** `{compile_id}`  \n"
        f"**Generated:** {generated_at}  \n"
        f"**Readiness:** **{score} / 100** &nbsp;·&nbsp; **{band_marker}**\n\n"
        f"> *Auto-generated by SOWSmith from the OrbitBrief envelope. "
        f"Every claim in this document traces back to a specific atom in "
        f"the source artifacts. Sections marked `[NEEDS DATA]` flag fields "
        f"the substrate could not satisfy from current evidence.*"
    )
...
def _section_footer(envelope: dict[str, Any]) -> str:
    ...
    return (
        "---\n\n"
        f"_Generated by SOWSmith ({SOW_VERSION}) from the OrbitBrief envelope "
        f"`{envelope.get('compile_id', '—')}` across "
        f"{summary.get('artifact_count', 0)} source artifacts. Receipt-grounded: "
        f"every claim above traces to a specific atom in the source bundle. "
        f"Audit the substrate at `orbitbrief.input.json` for the full atom / "
        f"edge / packet graph._"
    )
