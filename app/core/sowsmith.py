"""SOWSmith — deterministic Statement of Work generator from the envelope.

Consumes the OrbitBrief envelope (built by ``build_orbitbrief_envelope``)
and emits a contract-grade Statement of Work in markdown. Every clause
in the output traces back to a specific atom ID in the envelope, so
nothing the SOW says is unmoored from the source artifacts.

This is the deterministic substrate of the SOWSmith family the
Purpulse one-pager attributes to ``SOWSmith.scope_clause`` /
``SOWSmith.exclusion_clause``. An LLM polish layer can run on top —
this module produces the structural backbone and the receipts.
...
Design principles:
  * Every section pulls FROM the envelope's pre-computed cockpit
    ...
    stakeholder_load, project_vitals). No re-deriving here — the
    Core surfaces are the truth.
  * Every claim carries its source atom IDs as inline footnotes the
    LLM polish layer (or a reviewer) can expand into clickable cites.
  * MISSING fields surface explicitly as ``[NEEDS DATA: field_id]``
    placeholders so a PM scanning the SOW sees the gaps instead of
    silently shipping incomplete contracts.
...
def _section_footer(envelope: dict[str, Any]) -> str:
    ...
    return (
        "---\n\n"
        f"_Generated by SOWSmith ({SOW_VERSION}) from OrbitBrief envelope "
        f"`{envelope.get('compile_id', '—')}` across "
        f"{summary.get('artifact_count', 0)} source artifacts. Receipt-grounded: every "
        "claim above traces to an atom ID in the source bundle. Audit the substrate at "
        "`orbitbrief.input.json` for the full atom / edge / packet graph._"
    )
