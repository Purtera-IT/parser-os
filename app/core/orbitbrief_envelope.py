...
def write_orbitbrief_envelope(
    *,
    ...
    """Write the envelope JSON, markdown, and a substrate-side SOW preview.

    Returns ``(json_path, markdown_path, sow_preview_path)``.  Defaults
    to writing under ``<project_dir>/.orbitbrief/`` so consumers know
    exactly where to look.  Pass ``out_dir`` to override.

    The ``sow_preview.md`` file is generated by
    ``app.core.sow_preview.build_sow_markdown`` from the envelope's
    pre-computed cockpit surfaces. It is NOT the real SOW — the real
    SOWSmith (in ``purpulse.app`` with the 707-field SRL + policy
    pack) assembles the contract-grade document downstream. This
    preview is a receipt-grounded sanity check so a reviewer running
    parser-os can see what the substrate captured before SOWSmith
    runs.
    """
    ...
    sow_preview_path = out_dir / "sow_preview.md"
    ...
    # Substrate-side SOW preview (NOT the real SOWSmith — that lives
    # in purpulse.app with the 707-field SRL + policy pack).
    from app.core.sow_preview import build_sow_markdown
    sow_preview_path.write_text(build_sow_markdown(envelope), encoding="utf-8")
    return json_path, md_path, sow_preview_path
