...
def write_orbitbrief_envelope(
    *,
    ...
    """Write the envelope as JSON, markdown, and the rendered SOW.

    Returns ``(json_path, markdown_path, sow_path)``.  Defaults to
    writing under ``<project_dir>/.orbitbrief/`` so consumers know
    exactly where to look.  Pass ``out_dir`` to override.

    The SOW is generated by ``app.core.sowsmith.build_sow_markdown``
    from the envelope's pre-computed cockpit surfaces, and is fully
    receipt-grounded — every claim cites the atom ID it came from.
    """
    ...
    # SOWSmith: deterministic SOW generation from the envelope.
    from app.core.sowsmith import build_sow_markdown
