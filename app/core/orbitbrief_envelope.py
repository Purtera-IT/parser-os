...
def write_orbitbrief_envelope(
    *,
    ...
    """Write the envelope JSON, markdown, and the SOWSmith-generated SOW.

    Returns ``(json_path, markdown_path, sow_path)``. Defaults to
    writing under ``<project_dir>/.orbitbrief/``. Pass ``out_dir`` to
    override.

    The ``sow.md`` file is generated by ``app.core.sowsmith.
    build_sow_markdown`` from the envelope's pre-computed cockpit
    surfaces. Every claim cites the atom ID it came from; missing
    SRL fields surface as ``[NEEDS DATA: field_id]`` placeholders.
    """
